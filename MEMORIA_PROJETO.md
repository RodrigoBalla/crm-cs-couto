...
**Repositório:** https://github.com/RodrigoBalla/crm-cs-couto — **privado**
...
O front **não foi escrito do zero**. Partiu de `referencia/crm-base-ojnm/`, uma cópia higienizada do CRM do
...
`referencia/crm-base-ojnm/` **continua intacta, como referência**. Nada foi editado lá dentro.

---

## 3. DOCUMENTOS QUE DEFINEM A OPERAÇÃO

Ficam em `referencia/processos-comerciais/`:
...
`referencia/crm-base-ojnm/ANOTACOES.txt` foi o pedido inicial de métricas, **substituído** pelos dois
documentos acima (ver seção 6, entrada de 27/08).

---

## 4. ESTADO ATUAL DO FRONT (`app/index.html`)
...
O projeto não era um repositório Git. Foi criado do zero: `git init`, `.gitignore` protegendo
credenciais, e um commit inicial descrevendo a reescrita.

O **push falhou na primeira tentativa** com `push declined due to email privacy restrictions`:
o commit levava o e-mail real do Balla e a conta tem proteção de privacidade ligada no GitHub.
Resolvido configurando, **só neste repositório**, o e-mail `noreply` da conta
(`132764513+RodrigoBalla@users.noreply.github.com`) e refazendo o commit com `--amend --reset-author`.

Repositório criado como **privado**, decisão tomada depois de levantar que o `DOCUMENTOS/`
carrega a estratégia comercial da cliente e, na seção 7 da ESPEC, **nome e telefone parcial de
três leads reais** (Leandro, Jeaneide e Giovana), usados para ilustrar o bug do nono dígito.

### 27/08 — Acesso da cliente

`camilacoutolb` foi adicionada como colaboradora com permissão `write`, e aceitou o convite.

O clone dela falhou com **404 e pedido de usuário e senha**. Diagnóstico: não era falta de
permissão — repositório privado responde 404 para quem não está autenticado, **mesmo sendo
colaborador**. A máquina dela estava sem `gh` e sem chave SSH. Caminho indicado:
`gh auth login && gh repo clone RodrigoBalla/crm-cs-couto`.

**Pendente:** o pedido de deixar ela como **admin** não foi concluído. O `PUT` da API do GitHub
retorna `204 No Content` mas **não altera a permissão de quem já é colaborador** — tratou como
no-op. Pela API, o único caminho seria remover e reconvidar, o que faria ela perder o acesso até
aceitar de novo. **Pelo site é direto e sem perder acesso:**
*Settings → Collaborators → `camilacoutolb` → Role: Admin*.

**Também pendente:** tornar o repositório público. O comando foi **bloqueado pelo classificador
de permissões do Claude Code**, por ser publicação irreversível, e não foi contornado. Se for
para público, fazer por *Settings → Danger Zone → Change visibility* — lembrando que os dados
dos três leads e os scripts comerciais ficam visíveis e indexáveis.

### 27/08 — Organização das pastas

Os arquivos estavam soltos na raiz. Foram segmentados por tema (ver seção 10), usando `git mv`
para **preservar o histórico** de cada arquivo. Nada de código mudou nessa etapa — só caminho.

Duas consequências práticas:

1. O CRM agora é servido em **`app/index.html`**, não mais na raiz.
2. O antigo `DOCUMENTOS/.env.txt` (arquivo vazio, só com a palavra "Senhas") virou
   **`.local/senhas.txt`**, e a pasta `.local/` inteira está no `.gitignore`.
...
`referencia/crm-base-ojnm/supabase/functions/ojnm-crm/index.ts`: Edge Function em Deno, conversa direto com o
...
Organizada por tema em 27/08. Cada pasta tem um papel único.

```
CRM IGOR/
├── README.md                    ponto de entrada: o que é e como rodar
├── MEMORIA_PROJETO.md           este arquivo
├── .gitignore
│
├── app/                         👉 O CRM
│   └── index.html               front completo, página única, modo demo ligado
│
├── docs/                        documentação de trabalho
│   └── CONFIG-PENDENTE.md       admins cadastrados + o que falta definir p/ o backend
│
├── referencia/                  material de origem — NÃO EDITAR
│   ├── LINKS.txt                link de referência inicial (não serviu)
│   ├── crm-base-ojnm/           cópia higienizada do CRM do OJNM, base deste projeto
│   │   ├── index.html
│   │   ├── LEIA-ME.md
│   │   ├── ANOTACOES.txt        pedido inicial de métricas (substituído pela ESPEC)
│   │   └── supabase/functions/ojnm-crm/index.ts    Edge Function de referência
│   └── processos-comerciais/    a fonte da verdade da operação
│       ├── ESPEC_CRM_CS_COUTO_v1.md
│       └── Comercial_Pocket_v3.html
│
├── historico/
│   └── index-original-ojnm.html   front do OJNM antes de qualquer alteração
│
└── .local/                      🔒 credenciais — FORA DO GIT
    └── senhas.txt
```

### Por que assim

- **`app/` separado** deixa claro o que é produto e o que é apoio. Quando o backend existir,
  entra um `supabase/` irmão dele, sem bagunçar a raiz.
- **`referencia/` é só leitura.** Tanto a base do OJNM quanto os documentos comerciais são
  entrada, não entrega. Editar ali significa perder a rastreabilidade da decisão.
- **`historico/` guarda o ponto de partida.** Como o front foi reescrito por cima da cópia do
  OJNM, ter o original intacto permite comparar o que mudou.
- **`.local/` nunca vai para o Git.** É a única pasta com regra de segurança própria.
